...
ListNode* createList(Vector<int> values){
    ...
    ListNode* head = new ListNode(start,nullptr);
    ListNode* temp =head;
    for(int i=1;i<values.size();i++){
        int tmp = values.get(i);
        ListNode* t = new ListNode(tmp,nullptr);
        temp->next = t;
        temp = temp->next;
    }
    //delete temp;
...
PROVIDED_TEST("TEST-FOR-Eq"){
    Vector<int>values = {4,5,2,1};
    ListNode* list = createList(values);

    Vector<int>temp1={4,5,2,1};
    EXPECT(areEquivalent(list,temp1));

    Vector<int>temp2={4,5,2};
    EXPECT(!areEquivalent(list,temp2));

    Vector<int>temp3={4,5,2,1,8,0};
    bool temp = areEquivalent(list,temp3);
    EXPECT(!temp);
}
