...
void duplicateNegatives(Queue<int>& q) {
    int temp = 0;//used to record the latest element;
    int flag = 1;
    int n = q.size();
    for (int i = 0; i < n; i++) {
        int val = q.dequeue();
...
PROVIDED_TEST("duplicateNegatives, Student_Test1") {
    Queue<int> q = {-3, -5, 10};
    Queue<int> expected = {-3, -3, -5, -5, 10};

    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}

PROVIDED_TEST("duplicateNegatives, Student_Test2") {
    Queue<int> q = {8, -5, 10};
    Queue<int> expected = {8, -5, -5, 10};
